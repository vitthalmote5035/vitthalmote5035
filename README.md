# Hi 👋, I'm Vitthal Mote

🛡️ Penetration Tester | Ethical Hacker (Beginner)  
🔵 SOC Analyst (Beginner)  
🎓 Computer Engineering Student (2023–2027)

I am an aspiring cybersecurity professional with hands-on experience in **security labs, Linux system administration, networking fundamentals, vulnerability scanning, and basic penetration testing**.  
I have practical exposure through internships, self-built labs, and automation-focused security projects.

I strongly believe in **learning by doing**, documenting my work, and following **ethical and legal security practices**.

---

## 🎯 Core Cybersecurity Focus

### 🔴 Penetration Testing / Ethical Hacking
- Vulnerability Scanning & Assessment
- Basic Network & Web Application Testing
- Linux Privilege Escalation (Beginner)
- Security Tools & Methodology
- Risk Analysis & Reporting

### 🔵 SOC Analyst / Blue Team
- Linux & Windows Log Analysis
- Threat Detection & Monitoring
- Incident Identification & Response (Basic)
- Security Automation using Bash & Cron
- System Hardening & Access Control

---

## 🛠️ Technical Skills & Tools

### 🔹 Operating Systems
- Linux (Kali Linux)
- Windows (Server & Client)

### 🔹 Cybersecurity & Networking
- Network Security Fundamentals
- Penetration Testing Basics
- SOC Monitoring Concepts
- Malware Analysis (Introductory)

### 🔹 Security Tools
- Nmap
- Fail2Ban
- iptables
- Syslog & Journalctl
- Wireshark (Basics)

### 🔹 Programming & Scripting
- Bash / Shell Scripting
- PowerShell (Basics)
- C (Academic)

### 🔹 Version Control
- Git
- GitHub

---

## 📂 Featured Cybersecurity Projects

🔹 **[Linux Security Monitoring & Threat Detection Automation](https://github.com/vitthalmote5035/linux-security-monitoring)**  
- Automated log monitoring & threat detection using Bash
- Cron-based security scans with email alerts
- Fail2Ban & iptables-based automated IP blocking
- Daily security report generation

🔹 **[Pentest Home Lab](https://github.com/vitthalmote5035/pentest-home-lab)**  
- Network scanning & enumeration
- Vulnerability identification
- Exploitation basics
- Professional pentest reporting

🔹 **[SOC Home Lab](https://github.com/vitthalmote5035/soc-home-lab)**  
- Log collection & analysis
- Alert investigation
- Incident response documentation

🔹 **[CTF Writeups](https://github.com/vitthalmote5035/ctf-writeups)**  
- TryHackMe & beginner CTF challenges
- Step-by-step methodology & learning notes

---

## 💼 Internship Experience

### Cyber Security Intern – Prodigy InfoTech  
📅 Dec 2025

- Worked on real-world cybersecurity labs and security tasks
- Monitored and analyzed network traffic for potential threats
- Performed basic vulnerability scanning & assessments
- Worked with Linux systems, networking fundamentals, and system administration
- Automated basic security tasks using Bash & PowerShell
- Learned penetration testing concepts, risk analysis, and security best practices
- Collaborated with team members while following security policies

---

## 🎓 Education
**Bachelor of Computer Engineering (Pursuing)**  
PCET’s & NMVPM’s Nutan College of Engineering and Research  
📅 2023 – 2027  
📊 SGPA: 7.3 (Semester IV)

---

## 📜 Certifications
- CISCO Ethical Hacker
- CISCO Introduction to Cybersecurity
- APISEC University – API Penetration Testing
- SecBlueTeam – Introduction to Penetration Testing

---

## 🧪 Practice Platforms
- TryHackMe
- Hack The Box
- 
---

## 📫 Contact
- 📧 Email: vitthalmote5035@gmail.com
- 💼 LinkedIn: www.linkedin.com/in/vitthal-mote-12821928b
- 🧑‍💻 GitHub: https://github.com/vitthalmote5035

⚠️ All activities are conducted in legal, ethical, and authorized environments only.
